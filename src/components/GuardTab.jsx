...
import { closeEntryAction, updateOutpassStatus } from "@/lib/actions";
...
export default function GuardPortal({filteredOutpasses,setOutpasses}) {
  ...
  const handleStatusUpdate = (id) => {
    setOutpasses(prevOutpasses =>
      prevOutpasses.map(outpass =>
        outpass.id === id ? { ...outpass,isOut: false } : outpass
      )
    );
  };
  ...
  return (
    <div className="container mx-auto p-4 ">
      <h1 className="text-2xl font-bold mb-4 text-center">GUARD Portal - Entry Close</h1>
...
function OutpassItem({ outpass, onStatusUpdate }) {
  ...
  const handleStatusUpdate = async (newStatus) => {
    setIsUpdating(true);
    const result = await closeEntryAction(outpass.id);
    if (result.success) {
  ...
  return (
    <Card key={outpass.id} className="overflow-hidden m-4">
            ...
            <Button 
              ...
              disabled={outpass.isOut === false || isUpdating}
              variant="success"
            >
              Close Entry
            </Button>
